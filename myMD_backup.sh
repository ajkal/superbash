...
_workdir_=/data/kalenkiewiczaj/simulations/
#_workdir_=/gne/research/workspace/josepsli/bench/amber/tomp/work
_sourcedir_=$PWD
...
OPTIONS:
  ...
  -s           Stepsize in picoseconds (default is 0.003)
...
while getopts w:g:j:l:k:n:pPcR;rms:t:x:i: OPTION
do
    case $OPTION in
        w) _water_=$OPTARG;;
        g) _gpus_=$OPTARG;;
...
if [ "$_cont_" = 'y' ]; then
  ...
  if [ "$mdfiles" -gt 0 ]; then
    ...
    # Slurm
    output=$(sbatch -J "$_name_.cont" \
      -n $_cores_ \
      --nodes=$_nodes_ \
      --gres=gpu:$_gpus_ \
      --mail-type=END \
      --workdir="$_dir_" \
      "$_sourcedir_/subscripts/cont.sh")
    cont_job=$(echo $output | cut -f4 -d' ')

  elif [ "$eqfiles" -eq 2 ]; then
    ...
    if [ "$eqfiles" -eq 1 ]; then
      ...
      output=$(sbatch -J "$_name_.equil" \
        ...
        --mail-type=END \
        --workdir="$_dir_" \
        "$_sourcedir_/subscripts/equil2.sh")
      equil_job=$(echo $output | cut -f4 -d' ')
 
    elif [ "$eqfiles" -eq 0 ]; then
