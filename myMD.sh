...
if [ -d $_dir_/PRODUCTION ]
    ...
    nvt=$( ls $_dir_/EQUILIBRATION/$_name_.nvt.crd | wc -l )
    npt=$( ls $_dir_/EQUILIBRATION/$_name_.npt.crd | wc -l )
fi
...
while [ "$_stage_" -ne "prod" ]; do
    case "$_stage_" in
        prod)
            if [ "$_nc_" -gt 0 && "$eqfiles" -eq 2 ]; then        
        ...
        npt)
            if [ "$eqfiles" -gt 1 ]; then
                echo "Found $eqfiles equilibration files"
                echo Warning: NPT equilibration may already be complete
                echo Consider canceling and restarting with --stage prod
            elif ["$eqfiles" -eq 1]; then
                jobname="$_name_.equil2"
                script="$_sourcedir_/subscripts/equil2.sh"
                coord="EQUILIBRATION/$_name_.equil.crd.1"
